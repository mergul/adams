...
export class PostsListComponent implements OnInit, OnDestroy {
  ...
  constructor(private router: Router, private winRef: WindowRef, private newsService: NewsService) {
  }
  ngOnInit() {
  ...
  set percentage(value: number) {
    ...
    if (this.isUp && !this.unsubscriber$.closed && this.postsBehaviorSubject.getValue().length > 9) {
      this.currentPage++;
      this.getStories();
    }
  }
  ...
  getStories = () => {
    this._postsList.pipe(takeUntil(this.destroy$), map(x => {
